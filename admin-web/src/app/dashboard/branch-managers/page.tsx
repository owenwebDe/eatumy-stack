...
export default function BranchManagersPage() {
    ...
    const fetchBranchManagers = async () => {
        try {
            ...
            console.error("Failed to fetch branchManagers", error);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleDeposit = async () => {
        ...
        try {
            ...
            if (selectedUser && updatedUser && updatedUser.id === selectedUser.id) {
                ...
                setBranchManagers(prev => prev.map(u => u.id === updatedUser.id ? { ...u, walletBalance: updatedUser.walletBalance } : u));
            } else {
                fetchBranchManagers();
            }
        } catch (error: any) {
    ...
    const handleVerifyKyc = async () => {
        ...
        try {
            ...
            setBranchManagers(prev => prev.map(u => u.id === selectedUser.id ? updatedUser : u));
        } catch (error: any) {
    ...
    const handleAddUser = async () => {
        if (!newUserName || !newUserMobile || !newUserEmail) return;
        ...
        try {
            ...
            toast.error(error.response?.data?.error || "Failed to add branch manager");
        } finally {
    ...
    const handleEditUser = async () => {
        ...
        try {
            ...
            setBranchManagers(prev => prev.map(u => u.id === selectedUser.id ? data : u));
        } catch (error: any) {
            console.error("Edit User Error:", error);
            toast.error(error.response?.data?.error || "Failed to edit branch manager");
        } finally {
    ...
    const handleTerminateUser = async () => {
        if (!selectedUser) return;
        const confirm = window.confirm("Are you sure you want to terminate this branch manager? They will no longer have access.");
        if (!confirm) return;
        setIsTerminating(true);
        try {
            const { data } = await api.put(`/users/${selectedUser.id}/terminate`);
            toast.success("Branch Manager terminated");
            setSelectedUser(data);
            setBranchManagers(prev => prev.map(u => u.id === selectedUser.id ? data : u));
        } catch (error: any) {
            console.error("Terminate User Error:", error);
            toast.error(error.response?.data?.error || "Failed to terminate branch manager");
        } finally {
    ...
    const handleDeleteUser = async () => {
        if (!selectedUser) return;
        const confirm = window.confirm("WARNING: This will permanently delete the branch manager and all their data! Proceed?");
        if (!confirm) return;
        setIsDeleting(true);
        try {
            ...
            setBranchManagers(prev => prev.filter(u => u.id !== selectedUser.id));
        } catch (error: any) {
            console.error("Delete User Error:", error);
            toast.error(error.response?.data?.error || "Failed to delete branch manager");
        } finally {
            setIsDeleting(false);
        }
    };

    const filteredUsers = branchManagers.filter(user =>
    ...
    return (
        <div className="space-y-8 pb-10 relative h-[calc(100vh-100px)] flex flex-col">
            <div className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold font-heading tracking-tight">Branch Managers</h1>
                    ...
                    <Dialog open={isAddUserOpen} onOpenChange={setIsAddUserOpen}>
                        <DialogTrigger asChild>
                            <button className="flex items-center gap-2 px-4 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-bold shadow-lg shadow-slate-900/10 hover:bg-slate-800 transition-colors">
                                <Plus className="h-4 w-4" /> Add Branch Manager
                            </button>
                        </DialogTrigger>
                        <DialogContent>
                            <DialogHeader>
                            ...
                            <DialogFooter>
                                <Button onClick={handleAddUser} disabled={isAddingUser || !newUserName || !newUserMobile || !newUserEmail} className="w-full font-bold h-12 rounded-xl">
                    ...
                    <div className="flex-1 overflow-y-auto p-2">
                        {isLoading ? (
                            <div className="flex flex-col items-center justify-center h-64 text-muted-foreground">
                                ...
                                <p className="text-sm font-medium">Loading Branch Managers...</p>
                            </div>
                        ) : filteredUsers.length === 0 ? (
                            <div className="flex flex-col items-center justify-center h-64 text-muted-foreground">
                                <Users className="h-12 w-12 mb-4 opacity-20" />
                                <p>No branchManagers found.</p>
                            </div>
                        ) : (
                                ...
                                <div className="p-5 bg-slate-900 text-white rounded-3xl border border-border/50 relative overflow-hidden">
                                    ...
                                    <div className="relative z-10">
                                        ...
                                        <div className="flex items-center gap-2 mb-6">
                                            <span className="text-3xl font-black">{formatINR(selectedUser.walletBalance || 0)}</span>
                                        </div>
                                        <div className="flex gap-2">
