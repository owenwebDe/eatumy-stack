...
$REMOTE_DIR = "/home/admins/shareholder-web"

Write-Host "Deploying Shareholder App to VPS ($VPS_IP)..." -ForegroundColor Cyan
...
tar -czf deployment.tar.gz --exclude=node_modules --exclude=.next --exclude=.git .
...
Write-Host "🌐 Shareholder App live at http://$VPS_IP:3000" -ForegroundColor Green
